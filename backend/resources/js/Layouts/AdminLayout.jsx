...
export default function AdminLayout({ header, children }) {
    ...
    return (
        <div className="min-h-screen bg-obsidian-950">
            ...
            {/* Sidebar — full-width on mobile, narrower on tablet */}
            <aside className={`fixed inset-y-0 left-0 z-50 w-full sm:w-72 bg-obsidian-900/95 backdrop-blur-xl border-r border-obsidian-600/20 transform transition-transform duration-300 ease-out lg:translate-x-0 safe-area-padding ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                {/* Brand gradient */}
                <div className="absolute inset-x-0 top-0 h-32 bg-gradient-to-b from-electric-600/8 to-transparent pointer-events-none" />

                <div className="relative flex h-16 items-center justify-between px-4 sm:px-6 border-b border-obsidian-600/15">
                    ...
                    <button onClick={() => setSidebarOpen(false)} className="lg:hidden p-2 text-obsidian-400 hover:text-white transition-colors touch-target">
                        <X className="h-5 w-5" />
                    </button>
                </div>

                <nav className="relative mt-6 px-3 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 12rem)' }}>
                        ...
                        {navigation.map((item) => {
                            ...
                            return (
                                <Link
                                    ...
                                    onClick={() => setSidebarOpen(false)}
                                >
                                    <Icon className={`h-5 w-5 flex-shrink-0 ${active ? 'text-electric-400' : ''}`} />
                                    {item.name}
                                    {active && (
                                        <div className="ml-auto w-1.5 h-1.5 rounded-full bg-electric-400 shadow-glow flex-shrink-0" />
                                    )}
                                </Link>
                            );
                        })}
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-obsidian-600/15">
                    <div className="flex items-center gap-3 px-3 py-2 rounded-xl bg-obsidian-800/40">
                        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-electric-500/20 to-electric-600/20 border border-electric-500/20 flex-shrink-0">
            ...
            <div className="lg:pl-72">
                ...
                <header className="sticky top-0 z-30 h-16 flex items-center justify-between border-b border-obsidian-600/15 bg-obsidian-950/80 backdrop-blur-xl px-4 sm:px-6 lg:px-8 safe-area-padding">
                    <button onClick={() => setSidebarOpen(true)} className="p-2 text-obsidian-400 hover:text-white transition-colors lg:hidden touch-target">
                    ...
                    <div className="relative flex-shrink-0">
                        <button
                            ...
                            className="flex items-center gap-3 rounded-xl px-3 py-2 text-sm text-obsidian-300 hover:bg-obsidian-800/50 hover:text-white transition-all duration-200 focus:outline-none touch-target"
                        >
